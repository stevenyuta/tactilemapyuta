function editpath(){
  ...
  $(document).off('mouseup').on('mouseup',function(event){ //mouseup終了時の処理
    while(arrIntervalCnt.length !== 0) {  //インターバル関数のストップ
      clearInterval(arrIntervalCnt.shift());
    }
    draw.select('.now_editing').removeClass('now_editing')
    editpath_hover();
  })
  set_contextMenu();
  var mx = 0 , my = 0;
...
function editpath_hover(mode){
  ...
  if(mode!=="off"){
    //くっつき状態のpathのホバーイベント設定
    draw.select('.connected').mouseover(function() {
      this.attr({
        'stroke' : PATH_SELECT_COLOR,
        'cursor' : 'pointer'
      });
      this.off('mousedown',editpath_mousedown).mousedown(editpath_mousedown); //path上でクリックしたらeditpath_mousedown関数が起動する
    })
    draw.select('.connected').mouseout(function() {
      ...
      this.off('mousedown',editpath_mousedown); //editpath_mousedown関数の消去
    })
    //バラバラ状態のpathのホバーイベント設定
      ...
      case 40: // ↓
        ...
        draw.select('.editing_target').each(function(i, children){
          if(this.hasClass('edit_circle')){
            var orig_cx = this.attr('cx') ,orig_cy =  this.attr('cy');
            var cx = 0 , cy = 0;
            ...
            var nears = getSimultaneouslyEdit_element(this);
            if(nears.afterPath){
...
function toFragmented(path){
  editpath_array.push(path.attr('id'));
  let max_frag_num = getMax_fragmented_Group_Number(); //connectedがもつfrag_numの最大数を取得する
  let dpoint = path.clear().array().settle();
  ...
  path.after(fragmented_PathGroup);
  for(let j = 0; j < dpoint.length - 1; j++){
    var new_path = fragmented_PathGroup.path().addClass('fragmented').addClass('SVG_Element').addClass('path');
    var circle = fragmented_CircleGroup.circle().addClass('edit_circle').front(); ////重ね順を一番前に
    new_path.attr({
      'fill' : 'none',
      'stroke' : PATH_SELECT_COLOR,
    ...
    if(new_path.attr('stroke-width') === 0){ //線幅が0の場合
      new_path.attr({
        'stroke-width': '2' ,
    ...
    if(j === 0){
      new_path.addClass('first_fragmentedPath');
      circle.addClass('first_circle');
    }
    if(j===dpoint.length - 2) new_path.addClass('last_fragmentedPath');
    if(dpoint[j+1][0]==="Z"){  //次の要素がZ要素である場合
      new_path.M({x: dpoint[j][1], y: dpoint[j][2]}).L({x: dpoint[0][1], y: dpoint[0][2]});
    }else{ //次の要素がZ要素でない場合
      new_path.M({x: dpoint[j][1], y: dpoint[j][2]}).L({x: dpoint[j+1][1], y: dpoint[j+1][2]});
      if(j === dpoint.length -2){
        var circle = fragmented_CircleGroup.circle().addClass('edit_circle').addClass('last_circle').front(); ////重ね順を一番前に
        circle.attr({
          'cx' : dpoint[j+1][1],
          'cy' : dpoint[j+1][2],
          'r' : CIRCLE_RADIUS/(2 * draw.viewbox().zoom),
...
//パスのイベントの登録
****************************************/
function fragPath_EventSet(){
  var clickCount = 0;
  draw.select('.fragmented').each(function(i,children){
    this.off('mousedown').on('mousedown',function(event){ //mousedownイベントの登録
      editpath_hover("off");
      if(!input_key_buffer[16] && !this.hasClass('editing_target') && event.button===0){
        draw.select('.editing_target').each(function(i,children){
      ...
      if(event.button===0){ //左クリック時
        ...
        // ダブルクリックの場合
          ...
          var new_path1 = draw.path().M({x: dpoint[0][1], y: dpoint[0][2]}).L({x: mx, y: my }).addClass('fragmented').addClass('SVG_Element').addClass('path');
          var new_path2 = draw.path().M({x: mx, y: my}).L({x: dpoint[1][1], y: dpoint[1][2] }).addClass('fragmented').addClass('SVG_Element').addClass('path');
          var circle = SVG.get('#fragmented_CircleGroup_' + String(this.parent().attr('fragmented_Group_Number'))).circle().addClass('edit_circle').back();
          new_path1.attr({
            'fill' : 'none',
            'stroke' : PATH_SELECT_COLOR,
            'stroke-linejoin' : 'round',
          ...
          this.after(new_path2).after(new_path1);
          for(let i = 0; i < Number(assignment_Number) + 1; i++){
            circle.forward();
          }
          this.remove();
          update_editElement();
...
function move_editing(){
  ...
  var init_x = mx, init_y = my; //クリックを行った点
  if(input_key_buffer[17]) editpath_mousemove('90degree',init_x,init_y);
  arrIntervalCnt.push($interval_move = setInterval(function(e){
        draw.select('.editing_target').each(function(i,children){
          if(this.hasClass('edit_circle')){
            var original_cx = this.attr('cx') , original_cy = this.attr('cy'); //クリックを行った点
            var cx = original_cx + mx - init_x , cy = original_cy + my - init_y;
            this.attr({'cx':cx}) , this.attr({'cy':cy}); //円の位置を格納
            nears = getSimultaneouslyEdit_element(this);
            if(nears.beforePath){
              var dpoint = nears.beforePath.clear().array().settle(); //pathのdpoint配列を取得
              nears.beforePath.attr({'d':''}).M({x: dpoint[0][1], y: dpoint[0][2]}).L({x: cx, y: cy});
            }
            ...
            var dpoint = this.clear().array().settle(); //pathのdpoint配列を取得
            original_x1 = dpoint[0][1] , original_y1 = dpoint[0][2];
            original_x2 = dpoint[1][1] , original_y2 = dpoint[1][2];
            var x1 = original_x1 - init_x + mx , y1 = original_y1 - init_y + my;
            var x2 = original_x2 - init_x + mx , y2 = original_y2 - init_y + my;
            this.attr({'d':''}).M({x: x1, y: y1}).L({x: x2, y: y2});
            var nears = getSimultaneouslyEdit_element(this);
            ...
            if(nears.afterCircle) nears.afterCircle.attr({'cx':x2,'cy':y2});
            if(nears.beforePath){
              var dpoint = nears.beforePath.clear().array().settle();
              nears.beforePath.attr({'d':''}).M({x: dpoint[0][1], y: dpoint[0][2]}).L({x: x1, y: y1});
            }
            if(nears.afterPath){
              var dpoint = nears.afterPath.clear().array().settle();
              nears.afterPath.attr({'d':''}).M({x: x2, y: y2}).L({x: dpoint[1][1], y: dpoint[1][2]});
            }
          }
        })
...
function delete_editpath(){
  while(draw.select('.editing_target').first()){
    ...
    if(editing_target.hasClass('edit_circle')){
      var nears = getSimultaneouslyEdit_element(editing_target); //circleの近傍要素を取得
      let new_path;
      if(nears.afterPath)aP_dpoint =  nears.afterPath.clear().array().settle(); //afterPathのdpoint配列を取得
      if(nears.beforePath)bP_dpoint =  nears.beforePath.clear().array().settle(); //beforePathのdpoint配列を取得
      if(nears.afterPath && nears.beforePath){
        new_path = draw.path().M({x: bP_dpoint[0][1], y: bP_dpoint[0][2]}).L({x: aP_dpoint[1][1], y: aP_dpoint[1][2]});
        new_path.addClass('fragmented').addClass('SVG_Element').addClass('path');
        new_path.attr({
          'fill' : 'none',
          'stroke' : PATH_SELECT_COLOR,
          'stroke-linejoin' : 'round',
        ...
        nears.beforePath.after(new_path);
      }
      ...
      if(!editing_target.parent().hasClass('close')){
        ...
        SVG.get('fragmented_CircleGroup_'+ String(current_fragmented_Group_Number)).removeClass('close');
      }
...
function node_connect_function(){
  ...
  var circle1 = connectCircle.circle1 , circle2 = connectCircle.circle2;
  if(circle1 && circle2){
    cash_svg(); //svgデータのcash
